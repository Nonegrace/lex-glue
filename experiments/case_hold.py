...
class DataTrainingArguments:
    ...
    task_name: str = field(
        default="case_hold", metadata={"help": "The name of the task to train on"}
    )
...
def main():
    ...
    elif config.model_type == "openai-gpt":
    ...
    if training_args.do_predict:
        predict_dataset = MultipleChoiceDataset(
            tokenizer=tokenizer,
            task=data_args.task_name,
            max_seq_length=data_args.max_seq_length,
            overwrite_cache=data_args.overwrite_cache,
    ...
    )  # 应该是 (batch_size, num_choices, sequence_length)
    print(
